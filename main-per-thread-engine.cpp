...
int get_thread_id() {
#ifdef _OPENMP
  return std::min(omp_get_thread_num(), 31);
#else
  return 0;
#endif
}
...
public:
  ...
  Vector getColor(const Ray &ray, int recursion_depth, int thread_id) {
    ...
    if (intersect(ray, P, t, N, object_id)) {
      ...
      if (objects[object_id]->mirror) {
        ...
        Ray reflection(P, v);
        return getColor(reflection, recursion_depth + 1, thread_id);
      ...
      Ray indir_ray(P2, w);
      Vector radiance = getColor(indir_ray, recursion_depth + 1, thread_id);
...
#pragma omp parallel for schedule(dynamic, 1)
  for (int i = 0; i < H; i++) {
    ...
    for (int j = 0; j < W; j++) {
      ...
      for (int k = 0; k < N; k++) {
        ...
        sum_color = sum_color + scene.getColor(ray, 0, thread_id);
      }
  ...
  std::printf("Render time: %.3f seconds\n", seconds);
